...
- ScanCtrlTask：消费命令队列，在“安全点”停硬件 → 重建点表/重编码帧缓冲 → 重启硬件；同时约 10ms 轮询面板三键走同一控制路径（PD3 循环切形状、PD4 启停切换、PD5 幅度 +10% 循环）。
...
- 面板三键（下降沿、10ms 轮询消抖）：PD3 短按循环切形状（圆→正方→长方→圆）、PD4 短按启动/停止切换、PD5 短按幅度 +10%（10%~100% 循环）
